...
<body>
  ...
  <div style="margin-bottom:4px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
    ...
    <a href="/mustangs" title="Adjacent-tier: Citation Mustang movement (up-purchase signal)" style="display:inline-block;background:#0f766e;color:#fff;padding:8px 18px;border-radius:6px;font-size:13px;font-weight:600;text-decoration:none;">
      🛩️ Mustangs
    </a>
...
<body>
  ...
  <div style="margin-bottom:4px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
    ...
    <a href="/mustangs" title="Adjacent-tier Mustang movement (all regions)" style="display:inline-block;background:#0f766e;color:#fff;padding:8px 18px;border-radius:6px;font-size:13px;font-weight:600;text-decoration:none;">
      🛩️ Mustangs
    </a>
    {_watch_pill_html()}
  </div>
...
def mustangs_page():
    """
    Adjacent-tier report: every Mustang (C510) landing in the last N days.
    Not ATLAS-eligible on their own — signal for up-purchase into a CJ AND
    market-sizing for a potential 510-family winglet product.
    """
    try:
    ...
    data = database.get_mustang_activity(days=days, limit=400)

    if data["total_flights"] == 0:
        empty_msg = (
            "<em>No Mustang activity in the last %d days.</em><br><br>"
            "This report is fed by FlightAware polling for C510 arrivals into US "
            "destinations. Traffic will appear as it comes in — Mustang flights "
            "are less frequent than CJ family, so give it a few days." % days
        )
        return f"""<!DOCTYPE html><html><head><meta charset="utf-8">
<title>Mustang movement — A320/737 Sightings</title>
<style>
  body {{ background:#0f172a;color:#e2e8f0;font-family:Arial,sans-serif;padding:32px;max-width:1200px;margin:0 auto; }}
  h1 {{ font-size:22px;margin-bottom:6px; }}
  .sub {{ color:#94a3b8;font-size:13px;margin-bottom:20px; }}
...
<body>
  <div class="nav"><a href="/">← Sightings</a> · <a href="/insights">📊 Insights</a></div>
  <h1>🛩️ Mustang movement <span style="color:#94a3b8;font-size:14px;font-weight:normal;">· adjacent-tier report</span></h1>
  <div class="sub">Citation Mustang (C510) landings · last {days} days · not in ATLAS scope</div>
  <div style="background:#1e293b;padding:20px;border-radius:8px;color:#94a3b8;">{empty_msg}</div>
</body></html>"""

    # Flight table rows
    flight_rows = ""
    for f in data["flights"][:200]:
        chain_badge = ""
        if f.get("chain_hit"):
            chain_badge = (
                ' <span title="This landing closes a fuel-stop chain — operator likely outgrowing airplane" '
                'style="background:#3b82f622;color:#60a5fa;border:1px solid #3b82f655;'
                'padding:1px 6px;border-radius:3px;font-size:10px;font-weight:600;">🎯 CHAIN</span>'
            )
        dist_str = f'{f["distance_nm"]:,} nm' if f.get("distance_nm") else "—"
        local  = f.get("arrived_local") or ""
        track  = (f'<a href="{f["tracking_url"]}" target="_blank" '
                  f'style="color:#60a5fa;font-size:11px;">track</a>' if f.get("tracking_url") else "")
        arrived_short = (f.get("arrived_utc") or "")[:16].replace("T", " ")
        flight_rows += (
            f'<tr>'
            f'<td style="color:#94a3b8;font-size:11px;">{arrived_short}<br>'
            f'<span style="font-size:10px;">{local}</span></td>'
            f'<td><a href="/tail/{f["tail"]}" style="color:#60a5fa;font-weight:600;text-decoration:none;">'
            f'{f["tail"]}</a>{chain_badge}</td>'
            f'<td style="font-size:12px;color:#e2e8f0;">{f["operator"] or "—"}</td>'
            f'<td>{f["origin"]}</td>'
            f'<td>{f["dest"]}</td>'
            f'<td style="text-align:right;font-variant-numeric:tabular-nums;">{dist_str}</td>'
            f'<td>{track}</td>'
            f'</tr>'
        )
    if len(data["flights"]) > 200:
        flight_rows += (
            f'<tr><td colspan="7" style="text-align:center;color:#94a3b8;padding:10px;">'
            f'… and {len(data["flights"]) - 200} more (raise `?days=` for a smaller window)'
            f'</td></tr>'
        )

    # Operator rollup
    op_rows = ""
    for o in data["by_operator"][:20]:
        chain_str = (f'<span style="color:#60a5fa;font-weight:600;">🎯 {o["chain_hits"]}</span>'
                     if o["chain_hits"] else '<span style="color:#94a3b8">0</span>')
        op_rows += (
            f'<tr>'
            f'<td style="font-weight:600;color:#e2e8f0;">{o["operator"]}</td>'
            f'<td style="text-align:right;font-variant-numeric:tabular-nums;">{o["tails"]}</td>'
            f'<td style="text-align:right;font-variant-numeric:tabular-nums;">{o["flights"]}</td>'
            f'<td style="text-align:right;font-variant-numeric:tabular-nums;">{chain_str}</td>'
            f'</tr>'
        )
    if not op_rows:
        op_rows = '<tr><td colspan="4" style="color:#94a3b8;padding:12px;">No operators yet.</td></tr>'
...
<head>
  ...
  <style>
    * {{ box-sizing:border-box; margin:0; padding:0; }}
    body {{ background:#0f172a; color:#e2e8f0; font-family:Arial,sans-serif; padding:24px; max-width:1300px; margin:0 auto; }}
    h1 {{ font-size:22px; font-weight:bold; margin-bottom:4px; }}
    h2 {{ font-size:14px; font-weight:600; color:#cbd5e1; margin:24px 0 10px; border-bottom:1px solid #1e3a5f; padding-bottom:6px; text-transform:uppercase; letter-spacing:1px; }}
    .sub {{ color:#94a3b8; font-size:13px; margin-bottom:20px; }}
    .nav a {{ color:#60a5fa; text-decoration:none; margin-right:16px; }}
    .stats {{ display:flex; gap:12px; margin-bottom:20px; flex-wrap:wrap; }}
    .stat {{ background:#1e293b; border-radius:8px; padding:14px 18px; min-width:130px; }}
    .stat .label {{ font-size:10px; color:#94a3b8; text-transform:uppercase; letter-spacing:1px; }}
    .stat .value {{ font-size:22px; font-weight:bold; margin-top:2px; }}
    .banner {{ background:#1e2d1e; border-left:3px solid #22c55e; border-radius:6px; padding:12px 16px; font-size:13px; color:#86efac; margin-bottom:20px; }}
    ...
    table.data tr:hover td {{ background:#0f1f35; }}
    .window-picker a {{ color:#60a5fa; margin-right:12px; text-decoration:none; font-size:12px; }}
    .window-picker a.active {{ color:#22c55e; font-weight:600; }}
    @media(max-width:900px) {{ .grid {{ grid-template-columns:1fr; }} }}
  </style>
</head>
<body>
  <div class="nav"><a href="/">← Sightings</a> · <a href="/insights">📊 Insights</a> · <a href="/prospects">🎯 ATLAS Prospects</a> · <a href="/mustang-insights">📈 Mustang Insights</a></div>
  <h1>🛩️ Mustang movement <span style="color:#94a3b8;font-size:14px;font-weight:normal;">· adjacent-tier report</span></h1>
  <div class="sub">Citation Mustang (C510) landings · last {days} days · not in ATLAS scope · 🎯 badge = closes a fuel-stop chain (up-purchase signal)</div>

  <div class="window-picker" style="margin-bottom:16px;">
    Window:
    <a href="/mustangs?days=7"   class="{'active' if days==7 else ''}">7 days</a>
    <a href="/mustangs?days=30"  class="{'active' if days==30 else ''}">30 days</a>
    <a href="/mustangs?days=90"  class="{'active' if days==90 else ''}">90 days</a>
    <a href="/mustangs?days=180" class="{'active' if days==180 else ''}">180 days</a>
  </div>

  <div class="banner">
    Mustangs are tracked as an <strong>up-purchase / adjacent-tier</strong> signal.
    A Mustang making fuel-stop chains (🎯) suggests the operator is outgrowing
    the airplane — warm CJ + ATLAS conversation. This report is separate from
    the ATLAS math on the rest of the dashboard.
  </div>

  <div class="stats">
    <div class="stat"><div class="label">Flights</div><div class="value">{data["total_flights"]}</div></div>
    <div class="stat"><div class="label">Unique tails</div><div class="value">{data["unique_tails"]}</div></div>
    <div class="stat"><div class="label">Operators</div><div class="value">{data["unique_operators"]}</div></div>
    <div class="stat"><div class="label">Chain hits</div><div class="value" style="color:#60a5fa;">{len(data["chains"])}</div></div>
  </div>
  ...
  <div class="grid">
    <div class="box">
      ...
      <table class="data">
        <thead><tr>
          <th>Arrived</th><th>Tail</th><th>Operator</th><th>From</th><th>To</th>
          <th style="text-align:right;">Distance</th><th></th>
        </tr></thead>
    ...
    <div class="box">
      ...
      <table class="data">
        <thead><tr>
          ...
          <th style="text-align:right;">Flights</th>
...
def mustang_insights_page():
    data = database.get_mustang_insights()

    if data.get("sparse"):
        return f"""<!DOCTYPE html><html><head><meta charset="utf-8">
<title>Mustang Insights — A320/737 Sightings</title>
<style>body{{background:#0f172a;color:#e2e8f0;font-family:Arial,sans-serif;padding:32px;max-width:1200px;margin:0 auto;}}
.nav a{{color:#60a5fa;text-decoration:none;margin-right:16px;}}h1{{font-size:22px;margin-bottom:6px;}}.sub{{color:#94a3b8;font-size:13px;margin-bottom:20px;}}
.box{{background:#1e293b;border-radius:8px;padding:20px;color:#94a3b8;}}
</style></head>
<body>
  <div class="nav"><a href="/">← Sightings</a> · <a href="/eu">EU Sightings</a> · <a href="/mustangs">🛩️ Mustangs</a> · <a href="/insights">📊 CJ Insights</a> · <a href="/eu-insights">EU Insights</a></div>
  <h1>📈 Mustang Mission Intelligence</h1>
  <div class="sub">Citation Mustang (C510) · analytics · up-purchase signal</div>
  <div class="box">
    <p><strong>{data.get("total_flights", 0)} Mustang flights</strong> recorded across <strong>{data.get("unique_tails", 0)} tails</strong> so far.</p>
    <p style="margin-top:10px;">Distance data is still accumulating — come back once more flights have been ingested with origin/dest pairs.
    Charts and operator stats unlock at 5+ distance-tagged flights.</p>
  </div>
</body></html>"""

    # Pre-process template data
    dist_labels = _json.dumps(data["dist_hist_labels"])
    dist_data   = _json.dumps(data["dist_hist_data"])
    dist_pct    = _json.dumps(data["dist_hist_pct"])
    dur_labels  = _json.dumps(data["dur_hist_labels"])
    dur_data    = _json.dumps(data["dur_hist_data"])
    hour_data   = _json.dumps(data["hour_dist"])
    hour_pct    = _json.dumps(data["hour_dist_pct"])
    hour_lbl    = _json.dumps([f"{h:02d}:00" for h in range(24)])

    orig_lbl    = _json.dumps([x["icao"]  for x in data["top_origins"]])
    orig_cnt    = _json.dumps([x["count"] for x in data["top_origins"]])
    dest_lbl    = _json.dumps([x["icao"]  for x in data["top_dests"]])
    dest_cnt    = _json.dumps([x["count"] for x in data["top_dests"]])
    route_lbl   = _json.dumps([x["route"] for x in data["top_routes"]])
    route_cnt   = _json.dumps([x["count"] for x in data["top_routes"]])

    baseline_nm = data["baseline_nm"]
    pct_signal  = round(100 * (data["hot"] + data["warm"]) / data["total_flights"]) if data["total_flights"] else 0
    dur_str     = f"{data['avg_duration_h']}h avg" if data.get("avg_duration_h") else "—"
    kts_str     = f"{data['avg_ground_kts']} kts block" if data.get("avg_ground_kts") else ""

    # Operator leaderboard rows
    op_rows = ""
    for i, op in enumerate(data["operator_board"], 1):
        chain_cell = (f'<span style="color:#60a5fa;font-weight:700;">{op["chain_hits"]}</span>'
                      if op["chain_hits"] else '<span style="color:#475569;">0</span>')
        op_rows += (
            f'<tr>'
            f'<td style="color:#94a3b8;text-align:center;">{i}</td>'
            f'<td><strong style="color:#e2e8f0;">{op["operator"]}</strong></td>'
            f'<td style="text-align:right;">{op["flights"]}</td>'
            f'<td style="text-align:right;">{op["tails"]}</td>'
            f'<td style="text-align:right;">{op["avg_dist"]:,} nm</td>'
            f'<td style="text-align:right;">{op["max_dist"]:,} nm</td>'
            f'<td style="text-align:right;">{chain_cell}</td>'
            f'</tr>'
        )
    if not op_rows:
        op_rows = '<tr><td colspan="7" style="color:#94a3b8;padding:16px;">No operator data yet.</td></tr>'
...
<head>
  ...
  <style>
    * {{ box-sizing:border-box; margin:0; padding:0; }}
    body {{ background:#0f172a; color:#e2e8f0; font-family:Arial,sans-serif; padding:24px; max-width:1300px; margin:0 auto; }}
    h1 {{ font-size:24px; font-weight:bold; margin-bottom:4px; }}
    h2 {{ font-size:14px; font-weight:600; color:#cbd5e1; margin:28px 0 12px; border-bottom:1px solid #1e3a5f; padding-bottom:6px; text-transform:uppercase; letter-spacing:1px; }}
    .sub {{ color:#94a3b8; font-size:13px; margin-bottom:20px; }}
    .nav {{ margin-bottom:20px; }} .nav a {{ color:#60a5fa; text-decoration:none; margin-right:16px; }}
    ...
    .chart-box h3 {{ font-size:12px; color:#94a3b8; text-transform:uppercase; letter-spacing:1px; margin-bottom:14px; }}
    ...
    .insight.green {{ background:#1e2d1e; border-left-color:#22c55e; color:#86efac; }}
...
<body>
  <div class="nav">
    <a href="/">← Sightings</a>
    <a href="/mustangs">🛩️ Mustang activity</a>
    <a href="/insights">📊 CJ fleet insights</a>
    <a href="/prospects">🎯 ATLAS Prospects</a>
  </div>
  <h1>📈 Mustang Mission Intelligence</h1>
  <div class="sub">Citation Mustang (C510) · {data["total_flights"]} flights · up-purchase signal + potential 510-winglet market data</div>

  <div class="stats">
    <div class="stat"><div class="label">Flights</div><div class="value">{data["total_flights"]}</div></div>
    <div class="stat"><div class="label">Unique tails</div><div class="value">{data["unique_tails"]}</div></div>
    <div class="stat"><div class="label">Operators</div><div class="value">{data["unique_operators"]}</div></div>
    <div class="stat"><div class="label">🎯 Chain hits</div><div class="value" style="color:#60a5fa;">{data["chain_hits"]}</div></div>
    <div class="stat"><div class="label">Avg distance</div><div class="value">{data["avg_distance"]} nm</div></div>
    <div class="stat"><div class="label">Max distance</div><div class="value">{data["max_distance"]} nm</div></div>
  </div>

  <div class="insight green">
    ✈️ Average Mustang mission: <strong>{data["avg_distance"]} nm</strong>
    {"· <strong>" + dur_str + "</strong>" if dur_str != "—" else ""}
    {"· <strong>" + kts_str + "</strong>" if kts_str else ""}
  </div>
  <div class="insight {"amber" if pct_signal >= 20 else ""}">
    🎯 <strong>{data["hot"]}</strong> flights exceed MCT baseline ({baseline_nm:,} nm) —
    <strong>{data["warm"]}</strong> more within 80%.
    <strong>{pct_signal}%</strong> of Mustang activity shows range-growth signal.
  </div>
  {"<div class='insight amber'>⚡ " + str(data['chain_hits']) + " fuel-stop chain" + ("s" if data["chain_hits"] != 1 else "") + " detected — operators likely outgrowing the airplane.</div>" if data["chain_hits"] > 0 else ""}

  <div class="grid">
    <div class="chart-box">
      <h3>Distance distribution (nm)</h3>
  ...
  <div class="chart-box" style="margin-bottom:20px;">
    <h3>Arrival hour (destination local time)</h3>
  ...
  <div style="background:#1e293b;border-radius:8px;padding:16px;margin-bottom:20px;">
    <table class="data">
      <thead><tr>
        ...
        <th style="text-align:right;">Flights</th>
        <th style="text-align:right;">Tails</th>
        <th style="text-align:right;">Avg nm</th>
        <th style="text-align:right;">Max nm</th>
  ...
  const CHART_DEFAULTS = {{
    ...
    scales: {{
      x: {{ ticks: {{ color: "#94a3b8", font: {{ size: 10 }} }}, grid: {{ color: "#1e3a5f" }} }},
      y: {{ ticks: {{ color: "#94a3b8", font: {{ size: 10 }} }}, grid: {{ color: "#1e3a5f" }} }},
    }},
  }};
  const BAR_COLOR  = "rgba(59,130,246,0.7)";
  const LINE_COLOR = "rgba(34,197,94,0.8)";

  new Chart(document.getElementById("distChart"), {{
    type: "bar",
    data: {{ labels: {dist_labels}, datasets: [{{
      data: {dist_data}, backgroundColor: BAR_COLOR,
      borderColor: "rgba(59,130,246,1)", borderWidth: 1,
    }}] }},
    options: {{ ...CHART_DEFAULTS, plugins: {{ ...CHART_DEFAULTS.plugins,
      tooltip: {{ callbacks: {{ label: (c) => c.raw + " flights (" + {dist_pct}[c.dataIndex] + "%)" }} }} }} }},
  }});

  new Chart(document.getElementById("durChart"), {{
    type: "bar",
    data: {{ labels: {dur_labels}, datasets: [{{ data: {dur_data}, backgroundColor: "rgba(168,85,247,0.7)", borderColor: "rgba(168,85,247,1)", borderWidth: 1 }}] }},
    options: CHART_DEFAULTS,
  }});

  new Chart(document.getElementById("hourChart"), {{
    type: "bar",
    data: {{ labels: {hour_lbl}, datasets: [{{ data: {hour_data}, backgroundColor: BAR_COLOR, borderColor: "rgba(59,130,246,1)", borderWidth: 1 }}] }},
    options: {{ ...CHART_DEFAULTS, plugins: {{ ...CHART_DEFAULTS.plugins,
      tooltip: {{ callbacks: {{ label: (c) => c.raw + " arrivals (" + {hour_pct}[c.dataIndex] + "%)" }} }} }} }},
  }});

  new Chart(document.getElementById("origChart"), {{
    type: "bar", indexAxis: "y",
    data: {{ labels: {orig_lbl}, datasets: [{{ data: {orig_cnt}, backgroundColor: "rgba(34,197,94,0.7)" }}] }},
    options: CHART_DEFAULTS,
  }});

  new Chart(document.getElementById("destChart"), {{
    type: "bar", indexAxis: "y",
    data: {{ labels: {dest_lbl}, datasets: [{{ data: {dest_cnt}, backgroundColor: "rgba(251,146,60,0.7)" }}] }},
    options: CHART_DEFAULTS,
  }});

  new Chart(document.getElementById("routeChart"), {{
    type: "bar", indexAxis: "y",
    data: {{ labels: {route_lbl}, datasets: [{{ data: {route_cnt}, backgroundColor: "rgba(168,85,247,0.7)" }}] }},
    options: CHART_DEFAULTS,
  }});
  </script>
</body>
...
<body>
  <div class="nav"><a href="/">← Sightings</a> &nbsp;·&nbsp; <a href="/eu">EU Sightings</a> &nbsp;·&nbsp; <a href="/prospects">🎯 ATLAS Prospects</a> &nbsp;·&nbsp; <a href="/mustangs">🛩️ Mustangs</a> &nbsp;·&nbsp; <a href="/eu-insights">EU Insights</a></div>
